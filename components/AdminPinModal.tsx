...
const AdminPinModal: React.FC<AdminPinModalProps> = ({ isOpen, onClose, onPinSet, pinHash }) => {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-6 w-full max-w-sm transform transition-all relative">
        ...
        {stage === 'enterOld' && (
          <form onSubmit={handleOldPinSubmit} className="space-y-4">
            <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">기존 관리자 PIN 확인</h2>
            <div>
              <label htmlFor="old-admin-pin" className="sr-only">기존 PIN</label>
              <input
                id="old-admin-pin" type="password" value={oldPin} onChange={(e) => setOldPin(e.target.value)}
        ...
        {stage === 'setNew' && (
          <form onSubmit={handleNewPinSubmit} className="space-y-4">
            <h2 className="text-xl font-bold mb-4 text-gray-800 dark:text-white">{pinHash ? '새 관리자 PIN 설정' : '관리자 PIN 설정'}</h2>
            <div>
              <label htmlFor="new-admin-pin" className="sr-only">새 PIN</label>
              <input
                id="new-admin-pin" type="password" value={newPin} onChange={(e) => setNewPin(e.target.value)}
            ...
            <div>
              <label htmlFor="confirm-new-admin-pin" className="sr-only">새 PIN 확인</label>
              <input
                id="confirm-new-admin-pin" type="password" value={confirmPin} onChange={(e) => setConfirmPin(e.target.value)}
