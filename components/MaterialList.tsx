...
interface MaterialListProps {
  selectedItem: Keyword | null;
  onAddMaterial: () => void;
  onEditMaterial: (material: Material) => void;
  onDeleteMaterial: (id: string) => void;
}

const MaterialList: React.FC<MaterialListProps> = ({ selectedItem, onAddMaterial, onEditMaterial, onDeleteMaterial }) => {
  if (!selectedItem) {
    return (
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-800/50 p-4 sm:p-6 rounded-lg shadow-md h-full flex flex-col">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold text-gray-800 dark:text-white truncate" title={selectedItem.name}>
          {selectedItem.name}
        </h2>
